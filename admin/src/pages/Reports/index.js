        ...
        <List>
          {reports.map((item) => (
            ...
            >
              <ListItemAvatar>
                <Avatar
                  src={getUserAvatar(item.from.avatar, item.from.gender)}
                  alt=""
                />
              </ListItemAvatar>
              ...
              <div style={{ display: "flex" }}>
                ...
                <Menu
                  ...
                  keepMounted
                  open={!!u1Menu}
                  onClose={() => setU1Menu(null)}
                  TransitionComponent={Fade}
                >
                  <MenuItem onClick={() => openProfile(item.from.id)}>
                    Open Profile
                  </MenuItem>
                  <MenuItem onClick={() => openMail(item.from.username)}>
                    Send A Mail
                  </MenuItem>
                </Menu>
                ...
                <Menu
                  ...
                  keepMounted
                  open={!!u2Menu}
                  onClose={() => setU2Menu(null)}
                  TransitionComponent={Fade}
                >
                  <MenuItem onClick={() => openProfile(item.to.id)}>
                    Open Profile
                  </MenuItem>
                  <MenuItem onClick={() => openMail(item.to.username)}>
                    Send A Mail
                  </MenuItem>
                </Menu>
                ...
                <HoverOver title="Make answered">
                  <IconButton
                    hidden={!item.answered}
                    onClick={() => makeAnswered(item.id)}
                  >
                    <Visibility />
                  </IconButton>
                </HoverOver>
